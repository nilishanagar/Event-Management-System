...
function UserListRow(props) {
    ...
    const handleClick = () => {
        Axios.all([
            Axios.delete("https://event-management-system-backend-eyjc.onrender.com/eventRoute/delete-user/" + _id)
            .then((res) => {
                if (res.status === 200) {
            ...
            .then((eventResponse) => {
                if(eventResponse.status === 200){
                    ...
                    for(let i = 0; i < collectedEvents.length; i++){
                        ...
                        Axios.put("https://event-management-system-backend-eyjc.onrender.com/eventRoute/update-event/" + collectedEvents[i]._id, eventData)
                        .then((updateResponse) => {
